...
public class PoolManager : Singleton<PoolManager>
{
    ...
    public override void Awake()
    {
        ...
        {
            int preSpawn = poolList[i].cap;
            while (preSpawn > 0)
            {
                poolList[i].Spawn(Vector3.zero, Quaternion.identity);
                preSpawn--;

            }
            int unsp = poolList[i].cap;
            while (unsp > 0)
            {
                poolList[i].Unspawn(poolList[i].activeList[0]);
                unsp--;
            }
        }
    }
    ...
    {
        // if (poolList.Find(x => x.prefab.name.Equals(obj.name)) == null)
        // {
        //     Debug.LogError("Missing: " + obj.name);
        // }
        // else
        // {
        //     Debug.LogError("Existing: " + obj.name);
        // }
...
public class Pool
{
    ...
    public bool Unspawn(GameObject obj)
    {
        ...
        {
            obj.SetActive(false);
            obj.transform.SetParent(PoolManager.GetOPMTransform(), false);
    ...
    public void MatchObjectCount(int count)
    {
        ...
        for (int i = currentCount; i < count; i++)
        {
            ...
            obj.SetActive(false);
            obj.transform.SetParent(PoolManager.GetOPMTransform());
            inactiveList.Add(obj);
        }
    }
