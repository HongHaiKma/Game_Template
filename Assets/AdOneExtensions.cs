...
namespace AdOne
{
    public static class AdoneExtensions
    {
        public static Collider GetNearest(this Collider[] found, Vector3 pos)
        {
            Collider result = found[0];
            float sqrDis = (pos - result.transform.position).sqrMagnitude;
            for (int i = 0; i < found.Length; i++)
            {
                if (found[i] == null)
                    continue;

                if ((pos - found[i].transform.position).sqrMagnitude < sqrDis)
                    result = found[i];
            }
            return result;
        }

        public static Transform GetFirstInactiveChild(this Transform parent)
        {
        ...
        public static bool IsInside(this Vector3 me, Vector3 posBox, Vector3 sizeBox)
        {
            return !(me.x < posBox.x - sizeBox.x || me.x > posBox.x + sizeBox.x
                || me.y < posBox.y - sizeBox.y || me.y > posBox.y + sizeBox.y
                || me.z < posBox.z - sizeBox.z || me.z > posBox.z + sizeBox.z);
        }
        public static bool IsInside(this Vector2Int me, Vector2Int min, Vector2Int max)
        {
            return !(me.x < min.x || me.x > max.x || me.y < min.y || me.y > max.y);
        }

        public static Vector3 GetPositionOnArc(this Vector3 center, Vector3 forward, Vector3 right, float arcAngle, float radius, int total, int index)
        {
            if (total <= 1)
                return center + radius * forward;
            arcAngle = arcAngle * Mathf.PI / 180f;
            if (index <= (total - 1) * 0.5f)
            {
                float angle = arcAngle * 0.5f - index * (arcAngle / (total - 1));

                return center + radius * (forward - right * Mathf.Tan(angle));
            }
            else
            {
                float angle = arcAngle * 0.5f - (total - 1 - index) * (arcAngle / (total - 1));

                return center + radius * (forward + right * Mathf.Tan(angle));
            }
        }

        public static Vector3 GetPositionSurround(this Vector3 center, Vector3 forward, Vector3 right, float radius, int total, int index)
        {
            if (index < 0 || index > total - 1)
            {
                Debug.LogWarning($"Cannot find position, index is out of range! total = {total}; index = {index}");
                return center;
            }

            if (total == 1)
            {
                var rd = Random.value * 2f * Mathf.PI;
                return center + radius * new Vector3(Mathf.Sin(rd), 0f, Mathf.Cos(rd));
            }

            forward.Normalize();
            if (total == 0)
                return center + radius * forward;
            float angle = 2f * Mathf.PI * index / total;
            return center + radius * (forward * Mathf.Sin(angle) + right * Mathf.Cos(angle));
        }
        public static void DrawGizmosCube(Vector3 center, Vector3 extend, Quaternion rot)
        {
            Vector3 rotForward = rot * Vector3.forward;
            Vector3 rotUp = rot * Vector3.up;
            Vector3 rotRight = rot * Vector3.right;

            Vector3 blt = center + extend.x * (-rotRight) + extend.y * rotUp + extend.z * (-rotForward);
            Vector3 brt = center + extend.x * (rotRight) + extend.y * rotUp + extend.z * (-rotForward);
            Vector3 blb = center + extend.x * (-rotRight) + extend.y * (-rotUp) + extend.z * (-rotForward);
            Vector3 brb = center + extend.x * rotRight + extend.y * (-rotUp) + extend.z * (-rotForward);

            Vector3 flt = center + extend.x * (-rotRight) + extend.y * rotUp + extend.z * (rotForward);
            Vector3 frt = center + extend.x * (rotRight) + extend.y * rotUp + extend.z * (rotForward);
            Vector3 flb = center + extend.x * (-rotRight) + extend.y * (-rotUp) + extend.z * (rotForward);
            Vector3 frb = center + extend.x * rotRight + extend.y * (-rotUp) + extend.z * (rotForward);

            Gizmos.DrawLine(blt, brt);
            Gizmos.DrawLine(brt, brb);
            Gizmos.DrawLine(brb, blb);
            Gizmos.DrawLine(blb, blt);
            Gizmos.DrawLine(flt, frt);
            Gizmos.DrawLine(frt, frb);
            Gizmos.DrawLine(frb, flb);
            Gizmos.DrawLine(flb, flt);
            Gizmos.DrawLine(blt, flt);
            Gizmos.DrawLine(brt, frt);
            Gizmos.DrawLine(brb, frb);
            Gizmos.DrawLine(blb, flb);
        }

        public static RectTransform AsRectTransform(this Transform tr)
        {
            return tr as RectTransform;
        }

        public static Tween DoOpen(this RectTransform rct, TweenCallback actOnComplete = null)
        {
            rct.DOKill();
            rct.localScale = Vector3.zero;
            rct.gameObject.SetActive(true);
            return rct.DOScale(Vector3.one, 0.15f).SetEase(Ease.OutBack).SetUpdate(true).OnComplete(actOnComplete);
        }
        public static Tween DoClose(this RectTransform rct, TweenCallback actOnComplete = null)
        {
            return rct.DOScale(Vector3.zero, 0.15f).SetEase(Ease.InBack).SetUpdate(true).OnComplete(actOnComplete);
        }
